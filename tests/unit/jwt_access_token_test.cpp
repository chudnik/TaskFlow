...
TEST(JwtAccessToken, CreatesAndStrictlyValidatesClaimsAndCurrentState) {
  ...
  Validator validator;
  infrastructure::JwtAccessTokenService tokens{"test-signing-secret-with-at-least-32-bytes",
                                               "taskflow",
                                               "taskflow-api",
  ...
  const domain::User user{
      domain::Uuid::generate(),      "user@example.com", domain::GlobalRole::user,
...
TEST(JwtAccessToken, RejectsWrongIssuerAudienceSignatureAndExpiry) {
  ...
  infrastructure::JwtAccessTokenService wrong_issuer{"test-signing-secret-with-at-least-32-bytes",
                                                     "other",
                                                     "taskflow-api",
                                                     std::chrono::seconds{30},
                                                     clock,
  ...
  infrastructure::JwtAccessTokenService wrong_secret{"different-signing-secret-at-least-32-bytes",
                                                     "taskflow",
                                                     "taskflow-api",
                                                     std::chrono::seconds{30},
                                                     clock,
